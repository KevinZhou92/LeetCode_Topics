...
class Solution:
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        for col in range(minCol, maxCol + 1):
            verticalTraversals[col].sort(key=lambda nodeComb: (nodeComb[1], nodeComb[0]))
        
        res = [[nodeComb[0] for nodeComb in verticalTraversals[col]] for col in range(minCol, maxCol + 1)]
